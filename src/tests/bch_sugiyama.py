...
# This file was *autogenerated* from the file bch_sugiyama.sage
...
class BCHSugiyamaDecoder(Decoder):
    ...
    def _get_syndrome(self, received):
        ...
        b = self._defining_set[_sage_const_0 ]
    
        for i in range(b, b + _sage_const_2 *t):
    ...
    def decode_to_code(self, word):
        ...
        r_prev = x**(_sage_const_2 *t)
        r_act = sum([S[i]*x**i for i in range(len(S))])
        b_prev = _sage_const_0 
        b_act = _sage_const_1 
        ...
        degree_b_act = len(b_act_list) - _sage_const_1 
        
        # NO ERROR
        if(degree_b_act == _sage_const_0 ):
            return word

        a = self._primitive_root
        roots = b_act.roots()
        Xj = [_sage_const_1  / r[_sage_const_0 ] for r in roots]
        logger("Xj:", Xj)
        nu = len(Xj)
        ...
        matrix_e = matrix(nu, lambda i, j: entries_matrix(i, j, self._defining_set[_sage_const_0 ], Xj))

        syn = []

        for i in range(_sage_const_0 , nu):
            syn.append(S[i])
